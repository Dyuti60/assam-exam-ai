...
# T-049 — Add independent human review for SourceCandidates
...
## T-056 implementation note

Status: **Ready for review**.

Implemented immutable checksummed prompt versions, terminal structured execution audits, a provider-neutral coordinator/protocol, and an official Gemini SDK adapter behind that interface. Controlled provider failures after request preparation are audited; invalid input/template/model configuration is rejected before provider I/O. Provider I/O runs without an open database transaction, exact prompt identity is revalidated afterward, and terminal persistence commits once or rolls back fully.

Only prompt create/read and individual stored-audit retrieval are exposed over HTTP. No arbitrary prompt-execution API, generated knowledge/content, automated review/release/publication, embedding/RAG, scheduler, learner/public behavior, or T-057 was added.

Correction note (2026-09-12 Asia/Kolkata, UTC+05:30): bounded AI timeout, output-token, temperature, and model configuration through shared validators and immutable execution options; ended the initial prompt read through session close so successful and controlled-failure persistence performs no rollback; and expanded focused evidence for canonicalization, prompt immutability/concurrency, exact transactions, PostgreSQL constraints, and direct injected Gemini SDK error/cleanup/metadata/structured-output behavior. Developer-run local validation passed 84 focused and 638 full-suite tests with one existing warning. No live Gemini request, secret, approval, or T-057 work occurred. T-056 remains Ready for review.

Second correction note (2026-09-12 Asia/Kolkata, UTC+05:30): replaced caller-session coordination with coordinator-owned factory-created read/write sessions, deterministic close behavior, engine-level proof of zero checked-out connections during provider I/O, pre-commit response materialization, and no post-commit reload; hardened Gemini SDK response and error metadata access against hostile getters, non-sequences, unsafe scalar coercion, unbounded collections, and malformed usage; and aligned bounded request-identity validation across application and PostgreSQL metadata. Developer-run local validation passed 135 focused and 689 full-suite tests with one existing warning, plus all required regression groups, Ruff, dependency lock, fresh and seeded migration cycles, Alembic, PostgreSQL probes, secret, whitespace, and diff checks on dedicated `_test` databases. No live Gemini request, secret, approval, or T-057 work occurred. T-056 remains Ready for review.

Third correction note (2026-09-12 Asia/Kolkata, UTC+05:30): enclosed Gemini client construction, context entry, one generation call, complete response/metadata normalization, context exit, and cleanup in an ordinary-`Exception` boundary with stable specific mappings and the sanitized `AI_PROVIDER_ERROR` fallback; already-normalized errors remain unchanged and `BaseException` subclasses are not swallowed. Shared Settings/`AiExecutionOptions` validators now reject booleans before numeric coercion while retaining finite bounds and valid numeric environment strings. Direct injected coverage exercises arbitrary lifecycle failures and hostile accepted indexing, slicing, iteration, conversion, and element access with one call, no retry, and exact cleanup. Developer-run local validation passed 157 focused and 711 full-suite tests with one existing warning, plus 112 T-053 through T-055, 249 source-pipeline, 35 provenance/T-047, and 228 canonical/document/artifact regressions; changed/new-file Ruff, dependency lock, fresh and seeded migration cycles, Alembic head/check, direct PostgreSQL, secret, whitespace, and diff checks passed on dedicated `_test` databases. No live Gemini request, secret, approval, or T-057 work occurred. T-056 remains Ready for review.

Fourth correction note (2026-09-12 Asia/Kolkata, UTC+05:30): placed a true outer ordinary-`Exception` boundary around the inner Gemini lifecycle and all mapping/normalization helpers, closing the confirmed `_map_api_error` escape while preserving stable specific mappings, response-invalid normalization, `AiProviderError` pass-through, safe chaining, exact cleanup, and unswallowed `BaseException`. `_map_api_error` now guards code/status access and compares only exact bounded built-in primitives; injected tests cover hostile properties, `int`/`str` hashing and equality, integer comparison, `bool`, unsafe status/value conversion, jointly malformed metadata, and sensitive messages. Coordinator-level injected-Gemini evidence proves one call/no retry, no database connection during provider I/O, one sanitized terminal `FAILED` audit, one commit, zero rollbacks, exact cleanup, and no post-commit SQL. Developer-run validation passed 34 hostile/API-mapping tests, 181 complete T-056 tests, 112 T-053–T-055 regressions, 249 source-pipeline regressions, 35 provenance/T-047 regressions, 228 canonical/document/artifact regressions, and 735 full-suite tests with one existing warning. Ruff, lock, Alembic, fresh/seeded migration, PostgreSQL, secret, whitespace, and diff checks passed on dedicated `_test` databases. No live Gemini request or real secret was used. T-056 remains Ready for review, uncommitted, unpushed, and unapproved; T-057 was not defined or implemented.

Complete implementation-ownership audit note (2026-09-12 Asia/Kolkata, UTC+05:30): audited every T-056 boundary and retained the coordinator-owned session/transaction design plus the true outer Gemini ordinary-`Exception` boundary. Beyond the hostile mapper correction, exact scalar/result normalization now prevents generic providers from passing subclasses or unsafe token/cost/metadata values into parsing or PostgreSQL; arbitrary provider, JSON parser, and schema-normalization exceptions become stable sanitized failed audits; provider identities and error codes are closed and validated; prompt integer contracts are strict; numeric settings reject whitespace, malformed forms, booleans and subclasses while valid environment strings remain accepted; C1 request controls are rejected; and model identifiers share the exact 128-character constraint across Settings, options, ORM and migration. Exact exploit reproduction passed 3 tests, Gemini/mapping 106, complete T-056 213, T-053–T-055 112, source pipeline 249, provenance/T-047 35, canonical/document/artifact 228, and full suite 767 with one existing warning. Ruff, lock, Alembic head/check, fresh upgrade, seeded downgrade/re-upgrade, PostgreSQL constraints/restricted deletion, secret, whitespace and diff checks passed using only `_test` databases. No live Gemini call, real secret, GitHub CI claim, commit, push, approval, or T-057 work occurred. T-056 remains Ready for review.
