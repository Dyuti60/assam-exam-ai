...
A ContentPackage is a retained internal identity plus two independently position-ordered membership snapshots for one exact ContentVersion. Its current API is create-only and exposes no membership-mutation endpoint. `POST /api/v1/content-versions/{content_version_id}/content-packages` locks the ContentVersion and its currently RELEASED, exact-version NoteDraft and QuestionBankItem rows, rejects an entirely empty selection, and atomically stores only their IDs and positions. Composite PostgreSQL foreign keys prevent cross-ContentVersion membership, while primary-key, unique-position, and non-negative-position constraints protect exact ordering. Later withdrawal changes the dynamic T-030 manifest but never rewrites retained package membership. ContentPackage has no retrieval/list API, mutable metadata, approval, release, publication, file, PDF, delivery, learner, or AI behavior.
