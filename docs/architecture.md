...
This section describes only the repository inspected on 2026-09-09 in Asia/Kolkata (UTC+05:30). Test results recorded in `workflow.md` and `task_log.md` were run against dedicated PostgreSQL test databases.
...
| Migrations | Alembic is connected to application settings and `Base.metadata`; seventeen migrations exist, including Topic classification, provenance and approval foundations, sourced exam inputs, ContentVersion identity, versioned NoteDraft ownership, complete internal MCQ candidates, independent review and controlled release for candidates and drafts, and immutable ContentPackage identity with ordered membership snapshots and independent package review |
...
| Tests | Two hundred ten tests cover the foundation, ContentVersion ownership and complete internal MCQ-candidate constraints, sourced exam inputs, deterministic Topic priority, provenance, knowledge APIs, independent approval/release boundaries, version-scoped released-asset manifests, ContentPackage creation, retained-membership retrieval, exact-member stored-content expansion, independent package review and reset semantics, ordered immutable membership and exact ContentVersion agreement, PostgreSQL constraints, stored snapshots, locking, and transactional failure atomicity |
...
A ContentPackage is a retained internal identity plus two independently position-ordered membership snapshots for one exact ContentVersion. Its current API supports atomic creation, individual read-only retrieval, exact-member content expansion, and an independent DRAFT/APPROVED/REJECTED human-review decision; it exposes no list or membership-mutation endpoint. `POST /api/v1/content-versions/{content_version_id}/content-packages` locks the ContentVersion and its currently RELEASED, exact-version NoteDraft and QuestionBankItem rows, rejects an entirely empty selection, and atomically stores only their IDs and positions. `GET /api/v1/content-packages/{content_package_id}` returns those retained IDs in stored association-position order. `GET /api/v1/content-packages/{content_package_id}/content` expands exactly those IDs into existing full stored NoteDraft and QuestionBankItem responses, preserving independent package-member order and nested Claim/option order without copying or rebuilding content. `POST /api/v1/content-packages/{content_package_id}/approval` locks only the package row; APPROVED or REJECTED records a UTC decision timestamp and optional note, while DRAFT clears both. Composite PostgreSQL foreign keys prevent cross-ContentVersion membership, while primary-key, unique-position, and non-negative-position constraints protect exact ordering; separate checks enforce valid package review states and metadata. Package review never changes membership or member state and is independent of member review and release. Later withdrawal changes the dynamic T-030 manifest but never rewrites or hides retained package membership or its expanded T-033 view. ContentPackage has no list, release, publication, rendering, file, PDF, delivery, learner, or AI behavior.
