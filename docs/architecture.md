...
A ContentPackage is a retained internal identity plus two independently position-ordered membership snapshots for one exact ContentVersion. Its current API supports atomic creation, individual read-only retrieval, exact-member content expansion, independent DRAFT/APPROVED/REJECTED human review, a controlled UNRELEASED/RELEASED/WITHDRAWN lifecycle, and a read-only currently RELEASED collection; it exposes no general list or membership-mutation endpoint. Package creation stores exact released-asset membership, while the individual GET boundaries return retained membership IDs or exact stored member snapshots. Package approval and release decisions lock only the package row. An approved package with retained membership can move once from UNRELEASED to RELEASED and then to WITHDRAWN; withdrawal retains the original release time and prevents in-place re-release. `GET /api/v1/content-packages/released` filters only current RELEASED state in PostgreSQL, orders packages by ID, and eagerly loads both persisted-position membership lists without locks or writes. Member withdrawal, review changes, Claims, Verification, priority, and T-030 do not affect package eligibility or membership. PostgreSQL checks enforce review/release metadata, while composite foreign keys and position constraints preserve exact membership. ContentPackage has no approved collection, expanded collection, publication, rendering, file, PDF, delivery, learner, or AI behavior.
