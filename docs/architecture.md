...
| Tests | Two hundred one tests cover the foundation, ContentVersion ownership and complete internal MCQ-candidate constraints, sourced exam inputs, deterministic Topic priority, provenance, knowledge APIs, independent approval/release boundaries, version-scoped released-asset manifests, ContentPackage creation and individual retained-membership retrieval, ordered immutable membership and exact ContentVersion agreement, PostgreSQL constraints, stored snapshots, and transactional failure atomicity |
...
A ContentPackage is a retained internal identity plus two independently position-ordered membership snapshots for one exact ContentVersion. Its current API supports atomic creation and individual read-only retrieval; it exposes no list or membership-mutation endpoint. `POST /api/v1/content-versions/{content_version_id}/content-packages` locks the ContentVersion and its currently RELEASED, exact-version NoteDraft and QuestionBankItem rows, rejects an entirely empty selection, and atomically stores only their IDs and positions. `GET /api/v1/content-packages/{content_package_id}` returns those retained IDs in stored association-position order without rebuilding membership from current release or review state. Composite PostgreSQL foreign keys prevent cross-ContentVersion membership, while primary-key, unique-position, and non-negative-position constraints protect exact ordering. Later withdrawal changes the dynamic T-030 manifest but never rewrites or hides retained package membership. ContentPackage has no list, mutable metadata, approval, release, publication, file, PDF, delivery, learner, or AI behavior.
