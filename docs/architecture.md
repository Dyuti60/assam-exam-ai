...
| Migrations | Alembic is connected to application settings and `Base.metadata`; eighteen migrations exist, including Topic classification, provenance and approval foundations, sourced exam inputs, ContentVersion identity, versioned NoteDraft ownership, complete internal MCQ candidates, independent review and controlled release for candidates and drafts, and immutable ContentPackage identity with ordered membership snapshots, independent review, and controlled release |
...
| Tests | Two hundred fifteen tests cover the foundation, ContentVersion ownership and complete internal MCQ-candidate constraints, sourced exam inputs, deterministic Topic priority, provenance, knowledge APIs, independent approval/release boundaries, version-scoped released-asset manifests, ContentPackage creation, retained-membership retrieval, exact-member stored-content expansion, independent package review and controlled release, ordered immutable membership and exact ContentVersion agreement, PostgreSQL constraints, stored snapshots, locking, and transactional failure atomicity |
...
A ContentPackage is a retained internal identity plus two independently position-ordered membership snapshots for one exact ContentVersion. Its current API supports atomic creation, individual read-only retrieval, exact-member content expansion, independent DRAFT/APPROVED/REJECTED human review, and a controlled UNRELEASED/RELEASED/WITHDRAWN lifecycle; it exposes no list or membership-mutation endpoint. `POST /api/v1/content-versions/{content_version_id}/content-packages` locks the ContentVersion and its currently RELEASED, exact-version NoteDraft and QuestionBankItem rows, rejects an entirely empty selection, and atomically stores only their IDs and positions. The two GET boundaries return retained membership IDs or exact stored member snapshots. Package approval locks only the package row; APPROVED or REJECTED records a UTC decision timestamp and optional note, while DRAFT clears both. Package release also locks only the package row. An approved package with retained membership can move once from UNRELEASED to RELEASED and then to WITHDRAWN; withdrawal retains the original release time and prevents in-place re-release. A RELEASED package cannot be reset or rejected until withdrawal. PostgreSQL checks enforce valid review/release states and metadata, while composite foreign keys and position constraints preserve exact membership. Package release does not re-evaluate or change members, Claims, Verification, priority, or the T-030 manifest. ContentPackage has no list or released collection, publication, rendering, file, PDF, delivery, learner, or AI behavior.
