...
This section describes only the repository inspected on 2026-09-12 in Asia/Kolkata (UTC+05:30). Test results recorded in `workflow.md` and `task_log.md` were run against dedicated PostgreSQL test databases.
...
erDiagram
    ...
    SOURCE ||--o{ SOURCE_FETCH_RUN : "records attempts"
    SOURCE_FETCH_RUN ||--o| SOURCE_SNAPSHOT : "stores successful bytes"
    SOURCE_SNAPSHOT ||--o| SOURCE_EXTRACTION_RUN : "has one versioned extraction"
    SOURCE ||--o{ SOURCE_EXTRACTION_RUN : "retains provenance"
    SOURCE_EXTRACTION_RUN ||--|{ SOURCE_CHUNK : "owns ordered text slices"
...
`POST /api/v1/sources/{source_id}/fetch-runs` records one already-completed terminal Source fetch result without performing network work. FAILED runs retain a bounded stable error and no snapshot. SUCCEEDED runs atomically retain one immutable `SourceSnapshot` with exact supplied bytes, normalized supported content type, server-computed byte size, and lowercase SHA-256. Composite PostgreSQL references bind the requested URL to the Source and bind a snapshot's Source, requested/final URLs, and SUCCEEDED state to its run; named checks enforce terminal metadata, bytes, and checksum integrity. PostgreSQL cannot cheaply require every directly inserted SUCCEEDED parent to own a child, so the service constructs and commits that aggregate atomically. Read APIs return stored run/snapshot state under `no_autoflush` without hashing, repair, locks, or writes. This manual T-053 boundary remains network-free and does not extract content or mutate the Source.

`POST /api/v1/sources/{source_id}/fetch` is the separate synchronous executor. It copies the stored Source location, ends the lookup transaction, and then evaluates a dedicated hostname allowlist, strict HTTPS URL policy, all resolved A/AAAA addresses, IP-pinned hostname-verified TLS, redirect revalidation, fresh canonical robots policy, independent time/byte limits, and supported response media types. Every connection uses one already-validated global address and the intended hostname for SNI; redirects trigger a new full resolution and validation, which prevents an unvalidated second DNS lookup for each connection but does not claim DNS answers remain stable beyond that connection. Controlled failures are reduced to bounded stable codes and persisted as FAILED attempts. Successful exact bytes and metadata enter the same T-053 raw-byte persistence authority, which revalidates Source identity/location and commits one run/snapshot aggregate. The executor performs no sitemap traversal, extraction, downstream knowledge creation, Source mutation, or background work.

`POST /api/v1/source-snapshots/{source_snapshot_id}/extractions` applies the fixed `deterministic-text-v1` contract to one immutable stored snapshot without network access. It locally parses all six accepted snapshot media types, normalizes Unicode, line endings, spacing, controls, and blank lines deterministically, and persists either a terminal FAILED run with one sanitized code or a complete SUCCEEDED aggregate with a normalized-text character count/hash and position-ordered chunks. Each chunk is the exact normalized-text slice identified by stored character offsets and its own UTF-8 SHA-256. Fixed 1,000-character chunks use a 100-character overlap and prefer paragraph, newline, then whitespace boundaries in the latter half of the allowed window. Validated settings bound input bytes, normalized characters, and retained chunks.

The extraction service copies immutable snapshot bytes and provenance, ends the read transaction before local parsing, then revalidates snapshot identity, Source, content type, byte size, and checksum before one atomic flush/commit. A `(source_snapshot_id, extractor_key)` uniqueness constraint protects concurrent duplicates. Composite PostgreSQL references bind run provenance to the snapshot and every chunk to its successful run. PostgreSQL cannot cheaply require every successful parent to own a child without a circular/deferred design, so the service constructs that complete aggregate atomically. `GET /api/v1/source-extraction-runs/{source_extraction_run_id}` returns stored run/chunk state in relationship position order without reparsing, hashing, locks, or writes. PDF extraction uses constrained `pypdf` 6.x locally; encrypted, malformed, and textless documents fail deterministically, and OCR is absent.
...
Bounded official-source discovery, controlled fetch/snapshot persistence, and deterministic local snapshot extraction/chunking are implemented foundations; general research/search, downstream Evidence generation from chunks, semantic retrieval, AI verification, the full human-review workflow, content generation, QA, and customer-facing publication/delivery remain unimplemented. The internal manual knowledge API follows `route → schema → service/use case → repository → database`; it can create minimal Topics, optionally classify Claims by Topic, retrieve individual Evidence, link relevant Evidence to a Claim, retrieve the Claim with stable evidence IDs and separate verification/approval state, record a human approval decision, list only approved Claims in stable ID order, and retrieve a Verification with its Claim and ordered audit evidence provenance.
...
T-054 was independently reviewed after push at immutable commit `47872fea4b0c984d0c484c697edfad753eeae742`, whose exact parent is issuance commit `a821b18b6a768b762cc4bac73c462bc20adc7577`, and approved with no blocking finding. The reviewed boundary safely executes one allowlisted, robots-aware Source fetch without holding a database transaction across network I/O, then revalidates Source identity and records the terminal result through the immutable T-053 persistence authority. GitHub exposes no status contexts or workflow runs, so no CI pass is claimed. T-055 is issued separately for deterministic SourceSnapshot text extraction and ordered chunk persistence; it introduces no AI provider or API key.
...
T-057 is approved at implementation commit `901090625c11b005f486df8cf0693f77a139c260`, whose exact parent is issuance commit `1b35a20ef731edeb8939991e7fe2347d637c6246`.

T-058 is now issued, but not implemented, as a grounded Claim verification proposal agent. It will evaluate only a T-057-generated DRAFT/UNVERIFIED Claim against its exact stored Evidence and SourceChunk provenance through the T-056 provider-neutral execution authority. Its output must remain an immutable advisory proposal: it cannot create an existing `Verification`, change the Claim's verification summary or human-review state, or confer factual approval. T-059 remains undefined.
