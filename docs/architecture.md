...
| Tests | Two hundred five tests cover the foundation, ContentVersion ownership and complete internal MCQ-candidate constraints, sourced exam inputs, deterministic Topic priority, provenance, knowledge APIs, independent approval/release boundaries, version-scoped released-asset manifests, ContentPackage creation, retained-membership retrieval, and exact-member stored-content expansion, ordered immutable membership and exact ContentVersion agreement, PostgreSQL constraints, stored snapshots, and transactional failure atomicity |
...
A ContentPackage is a retained internal identity plus two independently position-ordered membership snapshots for one exact ContentVersion. Its current API supports atomic creation and individual read-only retrieval; it exposes no list or membership-mutation endpoint. `POST /api/v1/content-versions/{content_version_id}/content-packages` locks the ContentVersion and its currently RELEASED, exact-version NoteDraft and QuestionBankItem rows, rejects an entirely empty selection, and atomically stores only their IDs and positions. `GET /api/v1/content-packages/{content_package_id}` returns those retained IDs in stored association-position order. `GET /api/v1/content-packages/{content_package_id}/content` expands exactly those IDs into existing full stored NoteDraft and QuestionBankItem responses, preserving independent package-member order and nested Claim/option order without copying or rebuilding content. Composite PostgreSQL foreign keys prevent cross-ContentVersion membership, while primary-key, unique-position, and non-negative-position constraints protect exact ordering. Later withdrawal changes the dynamic T-030 manifest but never rewrites or hides retained package membership or its expanded T-033 view. ContentPackage has no list, mutable metadata, approval, release, publication, rendering, file, PDF, delivery, learner, or AI behavior.
...
T-031 is approved at implementation commit `a261a36a539c40b718e2185eaed794f700dd4b77`, with documentation correction commit `67e80c29928eb7e913bca99f899082562e4c2cb1`. The system can atomically persist immutable ContentPackage membership snapshots for one exact ContentVersion using currently RELEASED asset IDs and database-enforced ordered same-version provenance. T-032 is approved at commit `1a695d8a2335612ff4873df3a3c3bb51543d1591`; one retained ContentPackage can now be retrieved by ID with both membership lists in stored association-position order, independent of later asset state. T-033 is issued to add only a read-only expanded-content view of that retained membership for internal inspection and future deterministic rendering; package lifecycle, publication, PDF generation, download, learner access, AI, and personalization remain unimplemented.
