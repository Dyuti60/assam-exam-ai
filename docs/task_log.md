...
## T-056 implementation record

| Field | Value |
| --- | --- |
| Status | Ready for review |
| Scope | Provider-neutral prompt contracts, one-call structured AI coordination, immutable terminal audits, and isolated Gemini adapter; no generated domain records |
| API | Closed prompt-version create/read and individual execution-audit read; no HTTP execution or collection endpoint |
| Persistence | Migration `c9f2a6d4e817`, parent `b8f4e1c7d526`; prompt and terminal JSONB audit constraints preserve exact hashes and provenance |
| Provider | Official `google-genai` SDK constrained to 1.x behind `AiProvider`; explicit model, blank-key safe-disable, server-owned options, structured output and no retry |
| Transactions | Prompt read transaction ends before I/O; exact prompt is revalidated; one terminal audit commits once; persistence failures roll back and propagate |
| Validation | Developer-run local evidence: 84 focused and 638 full-suite tests passed with one existing warning; bounded configuration/options, canonicalization/immutability/concurrency, exact transaction counts, expanded PostgreSQL constraints, and direct injected Gemini SDK mapping/cleanup/metadata/output paths are covered; regressions, Ruff, lock, fresh/seeded migration, Alembic, secret, and diff checks passed on dedicated `_test` databases; no public Gemini call occurred |
| Boundaries | No Evidence/Claim/Verification or canonical-content generation, embedding/RAG, approval/release/publication, scheduler, learner/public API, infrastructure, or T-057 |

Second focused correction evidence (2026-09-12 Asia/Kolkata, UTC+05:30): the coordinator now owns separate factory-created read/write sessions and cannot alter a caller-owned unit of work; every owned session closes deterministically, provider I/O occurs after the read session closes with an engine-level assertion of zero checked-out connections, terminal responses are materialized before the single write commit, and no post-commit reload occurs. Gemini response normalization now treats inaccessible or unexpected SDK metadata as untrusted, bounded input and emits only stable sanitized errors. Request identity is constrained consistently in application, model, and migration metadata. Developer-run local validation passed 135 focused, 112 T-053–T-055, 249 source-pipeline, 35 provenance/T-047, 228 canonical/document/artifact, and 689 full-suite tests, each with the existing warning. Ruff, dependency lock, fresh and seeded migration cycles, PostgreSQL probes, Alembic head/check, secret, whitespace, and diff checks passed on dedicated `_test` databases. No live Gemini call occurred. T-056 remains Ready for review and is not approved; no T-057 work occurred.

Third focused correction evidence (2026-09-12 Asia/Kolkata, UTC+05:30): the Gemini adapter now encloses client construction, context entry, one generation call, complete response/metadata normalization, context exit, and cleanup in a catch-all ordinary-`Exception` boundary. Known failures retain specific stable codes, the fallback is `AI_PROVIDER_ERROR`, normalized errors pass through unchanged, raw details are suppressed, and `BaseException` subclasses are not swallowed. Shared Settings/`AiExecutionOptions` validators reject booleans before coercion and apply identical finite numeric bounds while accepting valid numeric environment strings. Direct injected tests cover arbitrary failures at every SDK lifecycle phase plus hostile accepted indexing, slicing, iteration, enum conversion, and element-property paths, with one call, no retry, and exact cleanup. Developer-run local validation passed 157 focused tests in 2.64s, 112 T-053 through T-055 in 3.85s, 249 source-pipeline in 6.56s, 35 provenance/T-047 in 2.79s, 228 canonical/document/artifact in 32.91s, and 711 full-suite tests in 36.43s, each with the existing warning. Changed/new-file Ruff, dependency lock, fresh and seeded migration cycles, direct PostgreSQL probes, Alembic head/check, secret, whitespace, and diff checks passed on dedicated `_test` databases. No live Gemini call occurred. T-056 remains Ready for review and is not approved; no T-057 work occurred.

Fourth focused correction evidence (2026-09-12 Asia/Kolkata, UTC+05:30): a true outer ordinary-`Exception` boundary now encloses the complete inner Gemini operation, including API-error mapping, so helper failures cannot escape sibling handlers; normalized `AiProviderError` values pass through, stable known mappings remain specific, response-normalization failures remain `AI_PROVIDER_RESPONSE_INVALID`, raw details are suppressed, and `BaseException` is not swallowed. `_map_api_error` defensively guards code/status access and uses only exact bounded built-in scalars, making hostile getters, `int`/`str` subclasses, hashing, equality, comparison, value access, conversion, malformed metadata, and sensitive upstream messages safe. Direct validation passed 34 hostile/API-mapping tests and 181 complete T-056 tests; coordinator-level injected-Gemini coverage proves one call/no retry, exact cleanup, no provider-I/O connection, one sanitized terminal `FAILED` audit, one commit, zero rollbacks, and no post-commit SQL. Required regressions passed 112 T-053–T-055, 249 source-pipeline, 35 provenance/T-047, 228 canonical/document/artifact, and 735 full-suite tests with one existing warning. Changed/new-file Ruff, lock, Alembic head/check, fresh upgrade, seeded downgrade/re-upgrade, and PostgreSQL constraints passed on dedicated `_test` databases. No live Gemini call or real secret was used. T-056 remains Ready for review and unapproved; T-057 was not defined or implemented.
