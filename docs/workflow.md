...
| --- | --- | --- | --- |
| `POST /api/v1/ai-prompt-versions` | `create_ai_prompt_version()` | `app/api/v1/routes/ai.py` | Stores one immutable, checksummed prompt contract; exact duplicate key/version maps to 409 |
| `GET /api/v1/ai-prompt-versions/{ai_prompt_version_id}` | `get_ai_prompt_version()` | `app/api/v1/routes/ai.py` | Returns the stored prompt contract without mutation |
| `GET /api/v1/ai-execution-runs/{ai_execution_run_id}` | `get_ai_execution_run()` | `app/api/v1/routes/ai.py` | Returns one stored terminal execution audit without provider I/O or revalidation |
...
### T-042 Immutable deterministic PDF artifact

- `POST /api/v1/content-documents/{content_document_id}/pdf-artifacts` accepts no body and returns metadata for one newly persisted artifact with HTTP 201. Missing, non-RELEASED, and duplicate documents use the stable 404/409 details recorded in the task.
- `deterministic-pdf-v1` is an isolated dependency-free renderer with fixed A4 geometry, margins, fonts, spacing, wrapping, pagination, object ordering, and fixed metadata. It renders only stored title and Markdown, rejects unsupported Unicode before persistence, and produces byte-identical output for identical supported input.
- `PdfArtifact` stores exact document/package/ContentVersion ownership, filename, fixed PDF media type, bytes, positive matching byte size, lowercase SHA-256, and creation time. Named PostgreSQL constraints enforce one artifact per document, composite ownership agreement, filename/media/checksum/byte integrity, and restricted document deletion.
- Creation locks only the target ContentDocument, checks current RELEASED eligibility and an ordinary duplicate before rendering, flushes constraints, commits once, reloads stored metadata, and rolls back renderer or persistence failures. Only the named uniqueness constraint is translated for a concurrent duplicate.
...
### T-043 PdfArtifact metadata retrieval and exact-byte download

- `GET /api/v1/pdf-artifacts/{pdf_artifact_id}` returns the existing stored metadata contract without `pdf_bytes`; `GET /api/v1/pdf-artifacts/{pdf_artifact_id}/download` returns the exact persisted bytes with stored PDF media type, byte size, and deterministic attachment filename. Both missing paths return `PdfArtifact <id> not found` with HTTP 404.
- Both service operations reuse one repository lookup by artifact primary ID. The query selects only `pdf_artifacts` under `no_autoflush`, with no joins, eager loads, row locks, writes, flushes, commits, regeneration, hashing, or current ContentDocument/package/member/Claim/Verification/priority evaluation.
- Stored metadata and download bytes remain available after document withdrawal and unrelated upstream state changes. The download route is registered before metadata retrieval and does not expose bytes through Pydantic JSON.
- Focused T-043 selection: 4 passed, 16 deselected, 1 warning in 1.08s. Complete PdfArtifact suite: 20 passed, 1 warning in 3.12s. Complete ContentPackage/ContentDocument suite: 61 passed, 1 warning in 9.52s. T-041 selection: 2 passed, 59 deselected, 1 warning in 1.30s. ContentVersion/T-030: 15 passed, 1 warning in 1.63s. NoteDraft suites: 43 passed, 1 warning in 3.26s. QuestionBankItem suites: 54 passed, 1 warning in 3.24s. Full suite: 269 passed, 1 warning in 19.78s.
- Fresh upgrade reached unchanged head `e7b4c9d2a615`; lock verification, changed-file Ruff, Alembic head/check, and diff checks passed. No model, schema, migration, renderer, dependency, API key, environment, configuration, Docker, external storage, or infrastructure change was required. T-043 is Ready for review, not approved; no list, ContentDocument-keyed lookup, lifecycle, publication, public/learner delivery, AI, mock assembly, personalization, or T-044 behavior was added.


### T-043 post-push independent review and T-044 issuance

- **APPROVED** at immutable implementation commit `b6ef5943536380ed8cf18a09133ad5cd68f603ad`, whose parent is the T-043 issuance head `45f4c71ed41d4dabc56278281182bed45d1800c0`.
- The implementation is exactly one commit over the issued base and changes only the expected route, repository, service, focused PdfArtifact tests, and current-state documentation.
- `/pdf-artifacts/{pdf_artifact_id}/download` is registered before the metadata route. Metadata reuses `PdfArtifactResponse` without bytes; download returns the exact stored bytes with stored media type, stored byte size, and attachment filename. Both missing paths preserve the stable `PdfArtifact <id> not found` 404 detail.
- Both operations share one repository lookup by PdfArtifact primary ID using a PdfArtifact-only SELECT under `no_autoflush`. They do not join/load related rows, lock, write, flush, commit, render, hash, or inspect current document/package/member/Claim/Verification/priority state.
- Focused tests prove exact metadata and byte/header fidelity, both missing paths, later document withdrawal and upstream-state independence, absence of renderer/hash calls, two isolated SELECTs for two requests, and no lock/flush/commit behavior.
...
### T-045 Controlled PdfArtifact release lifecycle

- Migration `a5d2c8f1e736` follows `f3c8a1d6e924`, adds only UNRELEASED/RELEASED/WITHDRAWN release metadata and named PostgreSQL status/lifecycle constraints, and migrates existing artifacts to UNRELEASED/null/null/null without inferred release or payload/review changes.
- `POST /api/v1/pdf-artifacts/{pdf_artifact_id}/release` accepts RELEASED or WITHDRAWN plus an optional note. Only the artifact's own APPROVED state permits initial release; withdrawal retains the original UTC release time, records a UTC withdrawal time, replaces the note, and is terminal.
- Release and approval decisions reuse the target-only PdfArtifact row lock. A currently RELEASED artifact must be withdrawn before DRAFT/REJECTED review changes. Success updates only the applicable lifecycle fields, commits once, and reloads through the ordinary lock-free lookup; persistence failures roll back.
- Related document/package/member/Claim/Verification state is not evaluated. Exact bytes, size, checksum, ownership, filename, media type, creation time, metadata retrieval, and ungated internal download remain unchanged.
...
### T-046 Released PdfArtifact delivery boundary

- `GET /api/v1/pdf-artifacts/released` returns stored `PdfArtifactResponse` metadata only for artifacts whose own current release state is RELEASED, ordered by ascending artifact ID. `GET /api/v1/pdf-artifacts/released/{pdf_artifact_id}/download` returns the exact persisted bytes with the stored media type, byte size, and attachment filename only while that exact artifact remains RELEASED.
- Both static released routes precede all dynamic PdfArtifact-ID routes. Each delegates through route, service, and repository layers to one PdfArtifact-only SELECT under `no_autoflush`; no related table is joined or loaded, and no lock, write, flush, commit, render, hash, or current related-state evaluation occurs.
- Missing and non-RELEASED released-download targets share the stable `PdfArtifact <id> not found` HTTP 404. The collection returns HTTP 200 with `[]` when no artifact qualifies. Existing ungated internal metadata/download, review, and release endpoints remain unchanged.
- Tests prove empty and ascending metadata results, exact stored metadata and bytes/headers, exclusion of UNRELEASED and WITHDRAWN artifacts, independence from later document/package/member/Claim/Verification state, withdrawal-driven removal, one-query/no-write behavior, and unchanged stored rows.
- Focused T-046: 4 passed, 51 deselected, 1 warning in 2.01s. Complete PdfArtifact: 55 passed, 1 warning in 8.77s. ContentPackage/ContentDocument: 61 passed, 1 warning in 9.73s. T-041 released documents: 2 passed, 59 deselected, 1 warning in 1.27s. ContentVersion/T-030: 15 passed, 1 warning in 1.64s. NoteDraft: 43 passed, 1 warning in 3.27s. QuestionBankItem: 54 passed, 1 warning in 3.29s. Full suite: 304 passed, 1 warning in 25.39s.
- Fresh upgrade reached unchanged head `a5d2c8f1e736`; changed-file Ruff, dependency-lock verification, Alembic head/check, and diff checks passed on dedicated `_test` databases. No model, schema, migration, renderer, dependency, API key, environment, configuration, Docker, external storage, or infrastructure change was required.
- T-046 is Ready for review, not approved. No general or approved artifact list, ContentDocument-keyed lookup, publication, public/learner delivery, external storage, AI, mock assembly, personalization, end-to-end smoke workflow, or T-047 behavior was added.


### T-046 post-push independent review and T-047 issuance

- **APPROVED** across immutable implementation commit `40c66d108b3c7b69ead420fd4e65a9de146f607d` and focused test-correction commit `2f7e5c5ea7e50f9ee533d9ba8bf9485aae305ca2`, over issuance base `7af933509ca583350d813f1540ad686eaee96489`.
- The implementation commit adds only the two released PdfArtifact routes plus their repository/service reads, focused tests, and current-state documentation. The correction commit changes only `tests/test_pdf_artifacts_api.py`.
- Both static released routes precede dynamic PdfArtifact-ID routes. The collection filters exact current RELEASED state and orders by artifact ID; released download returns exact stored bytes and stored headers, while missing and non-RELEASED targets share the stable 404.
- Each boundary performs one PdfArtifact-only SELECT under `no_autoflush`, without joins, relationship loading, locks, writes, flushes, commits, rendering, hashing, or related-state evaluation. Existing ungated internal endpoints remain compatible.
- The correction creates Evidence and a SUPPORTED Verification after artifact release, changes the Claim verification summary from UNVERIFIED to SUPPORTED, and proves both released boundaries and exact artifact, Evidence, Verification, and related state remain unchanged.
- Developer-recorded validation is 4 focused T-046 tests, 55 complete PdfArtifact tests, and 304 full-suite tests, each with one existing Starlette/httpx warning, plus successful Ruff, dependency-lock, Alembic head/check, fresh-upgrade, and diff checks. GitHub exposes no status contexts or workflow runs, so no CI pass is claimed.
- No model, schema, migration, renderer, dependency, environment, configuration, Docker, AGENTS.md, README, publication, external storage, public/learner delivery, AI, personalization, or T-047 implementation was included.
- T-047 is issued as the first complete automated internal deliverable smoke workflow, validating the existing trusted-content chain through a released exact-byte PDF without adding production behavior.

### T-047 Complete internal deliverable smoke workflow

- `tests/test_internal_deliverable_e2e.py` drives one coherent chain exclusively through existing HTTP APIs and the normal PostgreSQL dependency override: authoritative Source, Exam/Topic/sourced SyllabusVersion, exact ContentVersion, Evidence-linked and verified Claim, explicit Claim approval, version-owned NoteDraft and QuestionBankItem, independent asset review/release, released-assets manifest, immutable ContentPackage and expanded retrieval, package review/release, deterministic ContentDocument and retrieval/release, immutable PdfArtifact, artifact review/release, released metadata, and exact-byte released download.
- Assertions retain exact IDs through every provenance and ownership boundary, prove Verification remains separate from Claim approval, prove approval does not imply release at each canonical stage, and compare retained manifest/package content with the stored assets.
- The deterministic document assertion checks the exact rendered Markdown and its lowercase UTF-8 SHA-256. The PDF assertion checks stored ownership and metadata, absence of bytes from metadata, positive byte size, exact internal/released byte equality, exact stored headers, and matching byte length and lowercase SHA-256.
- Focused T-047: 1 passed, 1 warning in 0.86s. Complete PdfArtifact: 55 passed, 1 warning in 8.97s. ContentPackage/ContentDocument: 61 passed, 1 warning in 9.95s. Knowledge API: 29 passed, 1 warning in 1.20s. ContentVersion/released-assets: 15 passed, 1 warning in 1.63s. NoteDraft: 43 passed, 1 warning in 3.23s. QuestionBankItem: 54 passed, 1 warning in 3.44s. Full suite: 305 passed, 1 warning in 26.04s.
- Fresh `assam_exam_ai_t047_test` upgrade reached unchanged head `a5d2c8f1e736`; Ruff, dependency-lock verification, Alembic head/check, and diff/whitespace checks passed. The initial focused invocation before database creation ended at setup because the dedicated database did not exist; after its required creation and fresh upgrade, all validation passed.
- T-047 is Ready for review, not approved. No production code, model, schema, migration, renderer, dependency, API key, environment, configuration, Docker, SourceDiscoveryRun, learner/public behavior, publication, external storage, AI, orchestration, deployment, CI/CD, or T-048 behavior was added.


### T-047 post-push independent review and T-048 issuance

- **APPROVED** at immutable implementation commit `38eb63189c66167258ad2cbc54b5d9d3d79f4fe8`, whose exact parent is T-047 issuance commit `e18ad840ac98ef174289673c470266b092bf8b9e`.
- The commit adds only `tests/test_internal_deliverable_e2e.py` and permitted current-state documentation; no production module, model, schema, repository, service, route, migration, renderer, dependency, configuration, Docker, AGENTS.md, or README file changed.
- The smoke workflow uses FastAPI TestClient, the normal database dependency override, and a real dedicated PostgreSQL transaction. It uses existing HTTP APIs only—no mocks, stubs, or direct database inserts—and the deterministic PDF renderer executes normally.
- One coherent identity chain is asserted from Source, Exam, Topic, sourced SyllabusVersion, and ContentVersion through Evidence-linked Verification, explicit Claim approval, NoteDraft and QuestionBankItem provenance, released-assets manifest, retained package membership, deterministic ContentDocument, immutable PdfArtifact, and exact released-byte delivery.
- Verification remains distinct from human approval. NoteDraft, QuestionBankItem, ContentPackage, ContentDocument, and PdfArtifact each remain UNRELEASED after approval and require their own release transition.
- Exact Markdown UTF-8 SHA-256, PDF byte length/SHA-256, stored ownership, metadata without bytes, and internal/released byte/header equality are asserted.
- Developer-recorded validation is 1 focused T-047 test, 55 PdfArtifact tests, 61 ContentPackage/ContentDocument tests, 29 knowledge API tests, 15 ContentVersion/released-assets tests, 43 NoteDraft tests, 54 QuestionBankItem tests, and 305 full-suite tests, each with one existing warning, plus successful Ruff, lock, Alembic head/check, fresh upgrade, and diff checks. GitHub exposes no status contexts or workflow runs, so no CI pass is claimed.
- T-048 is issued as the persistence-only SourceDiscoveryRun and SourceCandidate foundation. External discovery, fetching, ingestion, review, LLMs, agents, embeddings, and API keys remain outside T-048.

### T-048 Source discovery audit snapshots

- `SourceDiscoveryRun` stores one normalized completed-attempt query, adapter key, SUCCEEDED/FAILED outcome, lifecycle-consistent error, and creation time. Ordered `SourceCandidate` rows retain normalized discovery leads and optional metadata without creating or linking a trusted `Source`.
- Migration `c8e4f2a9d617` follows `a5d2c8f1e736`. Named PostgreSQL checks enforce run and candidate lifecycle/content invariants; unique constraints enforce per-run positions and normalized locations; a composite restricted foreign key permits candidates only under their own successful run. Downgrade removes candidates before runs and leaves all earlier data unchanged.
- `POST /api/v1/source-discovery-runs` validates and atomically stores the supplied completed snapshot. `GET /api/v1/source-discovery-runs/{source_discovery_run_id}` returns the stored aggregate with candidates in persisted position order. Duplicate persisted position/location conflicts use the stable named-constraint HTTP 409 response; missing reads use the established 404 format.
- Routes delegate through `SourceDiscoveryRunCreate`/response schemas, `KnowledgeService`, and focused `KnowledgeRepository` add/read methods. Successful creation commits once and reloads the aggregate; any persistence failure rolls back all run/candidate rows. Retrieval uses a fixed run query plus select-in candidate query and performs no lock or write.
- Focused T-048: 30 passed, 1 warning in 0.99s. Existing Source/Evidence/Claim/Verification regressions: 34 passed, 1 warning in 1.40s. T-047 smoke workflow: 1 passed, 1 warning in 0.94s. Full suite: 335 passed, 1 warning in 30.15s.
- Fresh upgrade reached `c8e4f2a9d617`; a seeded `a5d2c8f1e736 -> c8e4f2a9d617 -> a5d2c8f1e736 -> c8e4f2a9d617` cycle preserved the pre-T-048 Source exactly and inferred no runs or candidates. Ruff, dependency-lock verification, Alembic head/check, and diff checks passed on dedicated `_test` databases.
- T-048 is Ready for review, not approved. No network discovery, adapter, fetch, review, Source promotion, ingestion, LLM, embedding, agent, learner/public behavior, dependency, configuration, Docker, infrastructure, or T-049 work was added.

### T-048 post-push independent review and T-049 issuance

- **APPROVED** at immutable implementation commit `f0590e414ed8386e755ced32dc2a508f2a53e35f`, whose exact parent is T-048 issuance commit `638836d35702461bebfe1fc3c9fc570fa25ee0e7`.
- The exact 13-file diff adds the two source-discovery models, registration, one migration, create/read schemas and layered API behavior, focused PostgreSQL tests, and current-state documentation only.
- Model and migration definitions agree on all fields and named constraints. The composite `(source_discovery_run_id, run_status)` reference permits candidates only for their own `SUCCEEDED` run, while restricted deletion and per-run position/location uniqueness preserve audit provenance.
- Creation normalizes inputs, assigns zero-based request positions, flushes the aggregate, commits once, rolls back failures, and translates only the two named uniqueness constraints. Retrieval uses one run query plus one ordered select-in candidate query without locks or writes.
- The existing curated `Source` boundary remains independent. A SourceCandidate is still an untrusted discovery lead; no network discovery, fetch, approval, Source promotion, ingestion, Evidence/Claim creation, AI, or learner behavior was added.
- Developer-recorded validation is 30 focused T-048 tests, 34 Source/Evidence/Claim/Verification regressions, 1 T-047 smoke test, and 335 full-suite tests, each with one existing warning. GitHub exposes no status contexts or workflow runs. Independent review reran static checks available without PostgreSQL; database execution remains developer evidence rather than CI evidence.
- T-049 is issued as the separate target-only human-review boundary for SourceCandidates. It must not fetch candidates, create Sources, or reinterpret approval as factual trust.

### T-049 Independent SourceCandidate human review

- Migration `d4a7c2e9f518` follows `c8e4f2a9d617` and adds only candidate `approval_status`, timezone-aware `approval_decided_at`, optional `reviewer_note`, and named status/lifecycle checks. Existing candidates become DRAFT/null/null without inference; downgrade removes only these T-049 objects.
- `POST /api/v1/source-candidates/{source_candidate_id}/approval` accepts DRAFT, APPROVED, or REJECTED. APPROVED/REJECTED store the current UTC time and optional note; DRAFT clears both. Missing candidates use the exact established 404 shape and invalid decisions return 422.
- The route delegates through `SourceCandidateApprovalCreate`, `KnowledgeService`, and focused repository methods. Approval locks exactly the target candidate with `FOR UPDATE OF source_candidates`, changes only three review fields, commits once, rolls back all persistence failures, and reloads through an ordinary lock-free candidate query.
- SourceDiscoveryRun create/get responses include current candidate review fields while retaining stored position order. Approval does not query or mutate the parent run or any Source, Evidence, Claim, Verification, content, document, or artifact state and never changes discovery snapshot fields.
...
- `POST /api/v1/source-candidates/{source_candidate_id}/promote` accepts explicit curated Source title, optional publisher, source type, authority tier, and license status. It copies the exact stored candidate location, forces `content_hash` to null, and returns HTTP 201 with the stored promotion snapshot and complete created Source response.
- The service locks only the target candidate and validates missing candidate, current APPROVED eligibility, and existing promotion in that order. It creates one new Source and one immutable `SourceCandidatePromotion`, commits once, rolls back every failure, maps only `uq_source_candidate_promotions_source_candidate_id` to the stable duplicate 409, and reloads the stored response.
- Migration `f6b2d8c4a731` follows `d4a7c2e9f518`. It adds supporting `(id, location)` uniqueness to candidates and Sources plus the promotion table. Composite restricted foreign keys enforce exact location agreement; named checks enforce nonblank location and an APPROVED decision snapshot; unique candidate and Source membership makes promotion one-to-one.
- Promotion snapshots the authorizing candidate decision time and reviewer note. Later candidate DRAFT, REJECTED, or reapproval decisions do not change the Source or promotion, and the T-050 collection remains governed only by current candidate review state.
...
- `POST /api/v1/sources/{source_id}/fetch-runs` accepts one closed discriminated SUCCEEDED or FAILED terminal result for the Source's exact stored location. It performs no network request. Missing Sources use the established 404; invalid terminal combinations, URL mismatch, invalid base64, unsupported content type, and oversized decoded bytes return 422 before persistence.
- SUCCEEDED creates one run and exactly one snapshot atomically. The service decodes exact bytes, enforces the configured 5 MiB maximum, and computes byte size and lowercase SHA-256. FAILED creates only a run with a stable bounded error code. Repeated attempts are independent and `Source.content_hash` is unchanged.
- `GET /api/v1/source-fetch-runs/{source_fetch_run_id}` and `GET /api/v1/source-snapshots/{source_snapshot_id}` return stored responses under `no_autoflush`, without locks, writes, hashing, reconstruction, or related-state evaluation.
...
- `POST /api/v1/sources/{source_id}/fetch` accepts no body and executes one independent synchronous attempt using only the exact stored Source location. Missing Sources retain the established 404; controlled policy, robots, network, HTTP and content failures return HTTP 201 with a sanitized FAILED run; success returns the existing run response with one immutable snapshot.
- Dedicated validated settings control allowed hosts, connect/read timeouts, robots and response byte limits, redirects and a fixed user-agent. The response limit cannot exceed the T-053 snapshot limit, and an empty allowlist safely disables execution.
- The generalized T-052 client revalidates HTTPS/default-port/host policy and every A/AAAA answer before every connection and redirect, pins the connection to a validated global IP while retaining hostname SNI/certificate verification, sends fixed headers, bounds reads, and closes response, connection, TLS and raw sockets on all paths. No ambient proxy is consulted.
- Each invocation fetches fresh canonical `/robots.txt` under the same client and user-agent before the Source URL. Robots denial, invalid/unavailable/oversized policy, URL/DNS/TLS/connect/timeout/redirect/transport failures, terminal non-2xx, unsupported content, empty bodies and oversized bodies map centrally to stable codes without storing upstream bodies, addresses or exception text.
...
### T-055 Deterministic SourceSnapshot extraction and ordered chunks

- `POST /api/v1/source-snapshots/{source_snapshot_id}/extractions` accepts no body and creates one terminal `deterministic-text-v1` extraction for an immutable snapshot. `GET /api/v1/source-extraction-runs/{source_extraction_run_id}` returns the identical stored run and position-ordered chunks without reparsing.
- `SourceExtractionRunResponse` and `SourceChunkResponse` expose exact snapshot/Source/checksum/extractor provenance, terminal metadata, normalized-text count/hash, stored chunk offsets/text/hashes, and UTC timestamps. Missing resources use established 404s; an existing snapshot/version pair uses the stable 409; controlled parser and limit failures return HTTP 201 as FAILED with no chunks.
- `source_extractor.py` isolates strict local text, visible-HTML, structural-order JSON, entity/DTD-rejecting XML, and `pypdf` 6.x PDF extraction. NFC, LF, NBSP/control, trailing-whitespace, blank-line, outer-space, and exactly-one-final-newline normalization is deterministic. Fixed 1,000-character chunks overlap by 100 characters and prefer paragraph, newline, then whitespace boundaries.
- Validated settings cap input bytes, extracted characters, and chunk count. Parsing occurs after ending the snapshot read transaction and before exact snapshot identity/provenance revalidation. The service constructs a complete terminal aggregate, flushes atomically, commits once, rolls back persistence failures, and maps only the named concurrent-duplicate constraint to 409.
...
- Added immutable checksummed `AiPromptVersion` contracts and terminal JSONB `AiExecutionRun` audit records under migration `c9f2a6d4e817` (parent `b8f4e1c7d526`). Named constraints enforce prompt provenance, terminal metadata, hashes, timing, tokens, reported cost metadata, uniqueness, and restricted deletion.
- Added closed prompt create/read APIs and read-only individual audit retrieval. No arbitrary HTTP execution endpoint or collection exists.
- `AiExecutionCoordinator` owns separate short-lived read and write sessions supplied by a factory. It copies the prompt then closes the read session before one provider call, revalidates with the write session, materializes the stored response before one commit, and closes the write session without a post-commit reload; it never commits, rolls back, closes, flushes, expires, or otherwise changes a caller-owned unit of work.
- The provider protocol isolates the official Google Gen AI SDK adapter. An inner boundary retains known specific mappings and response-invalid normalization, while a true outer ordinary-`Exception` boundary encloses construction, context entry, one generation call, API-error mapping, response normalization, context exit, and cleanup. Unexpected ordinary failures become `AI_PROVIDER_ERROR`, already-normalized `AiProviderError` values pass through unchanged, and `BaseException` subclasses are not swallowed. The API-error mapper is total for ordinary provider metadata: guarded properties and exact bounded built-in `int`/`str` values prevent hostile hashing, equality, comparison, value access, conversion, or raw details from escaping. The adapter closes its entered client context exactly once, never retries generation, and never returns or persists its key or raw provider details.
- Blank keys create an audited `AI_PROVIDER_DISABLED` result after safe request preparation and before outbound I/O. Invalid input/template/model configuration is rejected before I/O without an execution row.
- Complete implementation-ownership audit evidence: exact hostile mapper reproduction passed 3 tests; the complete Gemini/mapping failure matrix passed 106 tests; complete T-056 passed 215 tests; and the full suite passed 769 tests in 47.30s, each with one existing warning. The audit retained the true outer Gemini boundary and total exact-primitive API-error mapper, then additionally hardened exact response text/token handling, generic provider exception/error/result normalization, bounded representable cost and optional metadata, arbitrary JSON/parser/schema failures, provider identity before sessions/I/O, strict prompt integer contracts, numeric-string grammar/subclass rejection, C1 request-ID controls, and the shared 128-character model identifier constraint. Schema dumping and canonical hashing now complete in local temporaries before success output is assigned; any failure records `AI_OUTPUT_SCHEMA_INVALID` with null output fields. Safety metadata accepts only exact built-in JSON trees and is bounded against PostgreSQL-style rendered text, including the 410-entry case that is 3,281 compact bytes but 4,100 rendered bytes. The injected real-`GeminiProvider` coordinator regression proves one sanitized `FAILED` audit, one call/no retry, exact cleanup, one commit, zero rollbacks, no checked-out connection during I/O, and no post-commit SQL. Required regressions passed: 112 T-053–T-055, 249 source-pipeline, 35 provenance/T-047, and 228 canonical/document/artifact. Changed/new-file Ruff, lock, Alembic head/check, fresh upgrade, seeded downgrade/re-upgrade, and direct PostgreSQL constraints/restricted deletion passed on dedicated `_test` databases. No live Gemini call, real secret, GitHub CI claim, domain generation, or T-057 was added. T-056 remains Ready for review and unapproved.

### T-056 post-push review and T-057 issuance

- **APPROVED** at immutable implementation commit `34e7d2223a072cdf5d428616a2d6fbae39bc0537`, whose exact parent is T-056 issuance commit `b23d98b237c2880b8c17f828f04cd9c063339afa`.
- Independent reviewer `/root/t056_independent_review` returned `APPROVED` after inspecting the complete diff and rerunning the hostile-provider, session-ownership, structured-output, metadata-bound, migration, secret, and focused-test gates.
- Developer-local evidence: 215 focused T-056 tests, 112 T-053–T-055 regressions, 249 complete source-pipeline tests, 35 provenance/T-047 tests, 228 canonical/document/artifact regressions, and 769 full-suite tests passed with one existing warning. Ruff, dependency lock, Alembic head/check, fresh upgrade, seeded migration cycle, PostgreSQL probes, diff/whitespace, ignored-env, secret, and protected-stash checks passed on dedicated `_test` databases.
- Reviewer-local evidence on a separate `_test` database: 33 hostile tests passed with 182 deselected; all 215 focused T-056 tests passed; fresh upgrade, Ruff, dependency lock, Alembic head/check, diff, whitespace, final-newline, secret, and stash checks passed.
- No live Gemini request occurred, no real key was used or disclosed, and no GitHub CI result is claimed.
- T-057 is issued separately for one grounded Evidence and DRAFT Claim extraction agent over immutable SourceChunks and the T-056 execution authority. It must preserve exact source/chunk/prompt/provider/model/execution provenance and cannot automatically approve any Claim.

### T-057 Grounded Evidence and DRAFT Claim extraction agent

- Added closed `ClaimExtractionCreate`, versioned `GroundedClaimExtractionInput`/`GroundedClaimExtractionOutput`, and stored `ClaimExtractionRunResponse` contracts. `POST /api/v1/source-extraction-runs/{source_extraction_run_id}/claim-extractions` accepts only a strict positive PostgreSQL-range prompt-version ID; both route IDs use the same bound. `GET /api/v1/claim-extraction-runs/{claim_extraction_run_id}` returns the stored run.
- `ClaimExtractionService` copies one successful extraction, its contiguous position-ordered chunks, and one compatible immutable prompt under a short read session; validates configured chunk, character, UTF-8 byte, canonical-JSON, Claim, citation, and unique-Evidence limits; closes that session; and invokes `AiExecutionCoordinator` exactly once. Provider and structural-schema failures retain only their terminal AI audit and return a stable conflict. Exact semantic citation grounding is then validated against copied chunk slices using zero-based Python Unicode code-point offsets with inclusive start and exclusive end.
- After provider I/O the service revalidates the complete extraction/chunk identity and exact AI prompt/execution provenance. A valid result commits one `ClaimExtractionRun`, exact-slice Evidence records, DRAFT/UNVERIFIED Claims, ordinary Claim-Evidence links, and independently position-ordered provenance associations. A grounding failure records only a FAILED domain run. Unknown persistence failures roll back the complete domain aggregate without altering the already terminal AI audit.
- Migration `d7e3a9c5f218` follows `c9f2a6d4e817`. Named checks, unique constraints, and composite foreign keys enforce terminal state, exact extraction/snapshot/Source and prompt/execution agreement, citation ranges/checksums, independent positions, one domain run per AI execution, and restricted provenance deletion. PostgreSQL cannot cheaply prove cited substring equality, so the service validates exact slices before persistence.
