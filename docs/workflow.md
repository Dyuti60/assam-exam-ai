...
- Tests prove empty and ascending metadata results, exact stored metadata and bytes/headers, exclusion of UNRELEASED and WITHDRAWN artifacts, independence from later document/package/member/Claim state, withdrawal-driven removal, one-query/no-write behavior, and unchanged stored rows.
